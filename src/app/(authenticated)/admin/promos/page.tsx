...
import { deleteAdminPromo, fetchAdminDashboard, fetchAdminPromos, saveAdminPromo } from "@/lib/admin";
...
export default function AdminPromos() {
  ...
  const { data: promos = [] } = useQuery({
    ...
    enabled: adminContext?.permissions.canManagePromos ?? false,
  });

  async function save() {
    if (!editing) return;
    try {
      await saveAdminPromo(editing);
      toast.success("Saved");
      setOpen(false);
  ...
  return (
    <div>
      {adminContext && !adminContext.permissions.canManagePromos ? (
        <div className="rounded-xl border border-border bg-card p-8 text-center">
          <h1 className="font-display text-2xl">Promos are admin-only</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Promo codes and offers affect revenue, so this role cannot create or edit them.
          </p>
        </div>
      ) : (
      <>
